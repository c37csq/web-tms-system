  ...
  <div class="rounded-xl">
    <div class="pt-20px px-20px">
      <div class="flex items-center">
        <span class="text-secondary-300 text-lg mr-4">各快递订单量</span>
        <el-select size="small" placeholder="请选择周期" :model-value="curSelect" @change="onChange">
    ...
    <div ref="expressPieRef" :style="{ height: height * 1.6 + 'px', width }"></div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import { getClientPieData } from '@/api/home';
  ...
  const expressPieRef = ref<HTMLDivElement | null>(null);
  const curSelect = ref('0');
  const totalCount = ref(0);
  const { setOptions } = useECharts(expressPieRef as Ref<HTMLDivElement>);

  async function loadPieData() {
    const { tab1 } = await getClientPieData(unref(curSelect));
    return tab1;
  }
