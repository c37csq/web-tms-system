  ...
  <div class="rounded-xl">
    <div class="pt-20px px-20px">
      <div class="flex items-center">
        <span class="text-secondary-300 text-lg mr-4">各服务商订单量</span>
        <el-select size="small" placeholder="请选择周期" :model-value="curSelectDate" @change="onChangeServiceDate">
    ...
    <div ref="servicePieRef" :style="{ height: height * 1.6 + 'px', width }"></div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const servicePieRef = ref<HTMLDivElement | null>(null);
  const curSelectDate = ref('0');
  const totalCount = ref(0);

  const { setOptions } = useECharts(servicePieRef as Ref<HTMLDivElement>);

  async function loadPieData() {
    const { tab1 } = await getAdminPieData({ tab1: unref(curSelectDate), tab2: 0 });
    return tab1;
  }
  ...
  async function onChangeServiceDate(val) {
