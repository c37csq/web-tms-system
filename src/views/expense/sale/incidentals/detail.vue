...
<script lang="ts" setup>
  ...
  import {
    addSaleIncidentals,
    getSaleIncidentalsDetail,
    SaleIncidentalsParams,
    updateSaleIncidentals,
  } from '@/api/expense';
  import {
    useCurrencyList,
    useDictData,
  ...
  async function onChangeService(val) {
    ...
    const curService = unref(saleProductList).find((e) => e.promotingProductsCode === val);
    if (curService) {
      const { promotingProductsCode, teamCode } = curService;
  ...
  async function initial() {
    if (!!id) {
      try {
        const { promotingProductsCode, currency, startTime, endTime, sellProductsSundryChargesDetailsList } =
          await getSaleIncidentalsDetail(id);
        if (promotingProductsCode) {
          form.promotingProductsCode = promotingProductsCode;
          form.currency = currency;
          form.dateRange = [startTime, endTime];
          list.value = sellProductsSundryChargesDetailsList.map((e, idx) => {
          ...
          listId.value = sellProductsSundryChargesDetailsList.length + 1; // 改变唯一值
          saleProductList.value = await useSaleProductList();
          const curService = unref(saleProductList).find((e) => e.promotingProductsCode === promotingProductsCode);
          if (curService) {
          ...
          createMessage.error('加载销售杂费详情异常');
          await closeCurrent();
        }
      } catch (e) {
        console.error(e);
  ...
  async function operate(type) {
    switch (type) {
      ...
      case 'saveAs':
        ...
        const params: SaleIncidentalsParams = {
        ...
        // 处理列表数据
        params.sellProductsSundryChargesDetailsList = list.value.map((e) => {
        ...
        try {
          if (unref(isEdit)) {
            await updateSaleIncidentals({ ...params, sundryChargesId: id });
            createBriefSuccessMsg('编辑成功');
          } else {
            await addSaleIncidentals(params);
